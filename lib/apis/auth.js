...
const {httpThrow, httpValidate, httpAssert, getAuthorization,
  mergeUserToken, hashify} = require('./utils');
...
async function auth(ctx) {
  ...
  switch (body.strategy) {
    ...
    case 'session':
      ...
      if (!sess)
        ...
        if (sess.wid) {
          wechatUser = await wechatUsers.findById(sess.wid);
          if (!wechatUser)
  ...
  if (!newTokenPayload.wid && oldTokenPayload.wid) {
    wechatUser = await wechatUsers.findById(oldTokenPayload.wid);
    if (!wechatUser)
  ...
  if (!newTokenPayload.uid && newTokenPayload.wid && wechatUser.userId) {
    user = await users.findById(wechatUser.userId).notDeleted();
    if (!user) {
  ...
  if (!newTokenPayload.wid && newTokenPayload.uid && user.wechatId) {
    wechatUser = await wechatUsers.findById(user.wechatId);
    if (!wechatUser) {
...
const sendEmailSchema = ajv.compile({
  ...
  properties: {
    ...
    to: {type: 'string'},
    action: {type: 'string', enum: Object.keys(sendEmailTemplates)}
  },
  additionalProperties: false
});
...
async function sendEmail(ctx) {
  ...
  const token = session.genToken(),
    ...
    data = {email, username: body.emailUser, roles: ['user']};
  if (oldToken) {
    const sess = await session.loadAndRemove('user:' + oldToken);
    httpAssert(sess && sess.username === data.usernam, 400, {
      type: 'ESCHEMA',
      message: 'Wrong token'
    });
  ...
  const template = sendEmailTemplates[body.action](hashify(to));
...
async function fetchIdTsinghua(config, data) {
  const options = {
    jar: request.jar(),
...
async function validateTsinghua(ctx) {
  ...
  const {session, users} = ctx.models,
    data = users.mapIdTsinghua(await fetchIdTsinghua(ctx.config['tsinghua-proxy'], body));
  httpAssert(data, 401, {
    type: 'EAUTH',
...
async function bindWechat(ctx) {
  ...
  httpAssert(token && (token.role && token.role & 0b100 || (body.user === token.uid &&
  ...
  const [user, wechatUser] = await Promise.all([
    ...
    wechatUsers.findById(body.wechatUser)
  ]);
...
async function unbindWechat(ctx) {
  ...
  const token = await getAuthorization(ctx);
  httpAssert(token && (token.role && token.role & 0b100 || body.user === token.uid), 401, {
    type: 'EAUTH',
    message: 'Authentication failed'
  });
