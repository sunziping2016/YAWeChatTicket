...
module.exports = function (db, sio) {
  ...
  activitySchema.methods.toPlainObject = function () {
    const result = {};
    ['name', 'place', 'beginTime', 'endTime', 'bookBeginTime', 'bookEndTime',
      'description', 'excerption', 'totalTickets', 'soldTickets', 'createdAt',
      'updatedAt', 'published', 'deleted', 'creator', '_id'].forEach(x => {
        result[x] = this[x];
    });
    ['mainImage', 'titleImage', 'mainImageThumbnail', 'titleImageThumbnail'].forEach(x => {
      result[x] = path.join('uploads', this[x]);
    });
    return result;
  };
